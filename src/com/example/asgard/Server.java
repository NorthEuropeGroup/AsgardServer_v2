...
public class Server {
	//public String ip;
	//public int port;
	private String Server_ADDRESS="140.116.104.109";
	...
	private BattleField tempBF;
	private ArrayList<BattleField> BattleList;
	...
	public Server()
	{
		...
		BattleList = new ArrayList <BattleField>();
		S_outV = new ArrayList <ObjectOutputStream>();
		S_inV = new ArrayList <ObjectInputStream>();
	}
				...
				try{
					...
					while ((message = (MsgData) in.readObject())!=null)
					{
						...
						else if(message.getType().equals("ANSWER"))
						{
							...
							{
								tempBF = new BattleField(message.getReceiver(),message.getSender());
								BattleList.add(tempBF);
						...
						else if(message.getType().equals("CARDSELECT"))
						{
							...
							{
								if(message.getSender().equals(BattleList.get(i).IDs.get(0)) || message.getSender().equals(BattleList.get(i).IDs.get(1)))//the sender in this BF
								{
									BattleList.get(i).team.add(message.getBC());
									if(BattleList.get(i).team.size() == 2)
										BattleList.get(i).determineOrder();
	...
	//public void tellREADY(CardnMaster cm)
	//{
